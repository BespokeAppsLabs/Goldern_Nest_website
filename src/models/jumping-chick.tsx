...
export function JumpingChick(props: JSX.IntrinsicElements['group']) {
  ...
  useEffect(() => {
    ...
    try {
      console.debug(
        '[JumpingChick] GLB animation clips:',
        animations.map((c: any) => c.name),
      );
      console.debug(
        '[JumpingChick] useAnimations action keys:',
        Object.keys(actions),
      );
    } catch (err) {
    ...
    const findWalkAction = () => {
      ...
      const clipMatch = animations.find(
        (c: any) => c.name && String(c.name).toLowerCase().includes('walk'),
      );
      ...
      if (keys.length > 0) {
        console.warn(
          '[JumpingChick] No explicit "walk" animation found — falling back to first animation:',
          keys[0],
        );
