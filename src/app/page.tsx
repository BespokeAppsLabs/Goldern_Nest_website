      ...
      <main>
        ...
        <section className="py-12 md:py-16 lg:py-20 bg-white">
          <div className="container-width px-4 md:px-6">
            <div className="text-center mb-8 md:mb-12">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 md:mb-4 text-gray-900">
                {WhatCustomersSayContent.title}
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {WhatCustomersSayContent.testimonials
                .slice(0, 2)
                .map((testimonial) => (
                  <div
                    key={`${testimonial.author}-${testimonial.location}`}
                    className="p-6 md:p-8 bg-primary-50 rounded-2xl shadow-md"
                  >
                    <p className="text-base md:text-lg text-gray-700 mb-3 md:mb-4 italic">
                      "{testimonial.quote}"
                    </p>
                    <p className="font-semibold text-primary-700 text-sm md:text-base">
